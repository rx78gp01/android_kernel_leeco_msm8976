...
struct synaptics_rmi4_data {
	...
	spinlock_t lock;
	int pm_status;
	struct workqueue_struct *synaptics_pm_wq;
	struct delayed_work synaptics_pm_work;
	#ifdef PINCTRL
	struct pinctrl *synaptics_pinctrl;
...
//Log define
//#define SY_INFO(fmt,arg...)           printk("<<-synaptics-info->> "fmt"\n",##arg)
#if defined CONFIG_TOUCHSCREEN_DEBUG_INFO
#define SYNAP_INFO_ON 1
#else
#define SYNAP_INFO_ON 0
#endif

#define SYNAP_INFO(fmt, arg...)	do {\
		if (SYNAP_INFO_ON) {\
			printk("<<-SYNAP-INFO->> "fmt"\n", ##arg); } \
		} while (0)
